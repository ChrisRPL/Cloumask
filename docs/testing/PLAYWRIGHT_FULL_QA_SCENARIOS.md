...
- if both lifecycle `status` and `resume_status` metadata are corrupted, ignore lifecycle state and fall back to hydrated/local truth instead of surfacing invalid resume copy
