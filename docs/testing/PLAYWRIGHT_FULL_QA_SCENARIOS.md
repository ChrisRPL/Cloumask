...
- latest thread auto-resume selection priority (`awaiting review` before `in progress` before `failed` before `completed`/`ready`)
...
- thread-list lifecycle `status` stays normalized to `active`/`completed`/`cancelled`; `summary` is user-facing copy, `resume_status` drives selection/fallback, and resume UI should key off `summary`/`resume_status`
